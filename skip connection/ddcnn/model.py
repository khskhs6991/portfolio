    ...
    def forward(self, x):
        ...
        for i in range(6):
            ...
            for j in range(len(feature_maps) - 1):
                # asc = ASC(output.shape[1]+feature_maps[j].shape[1])
                # print(f'output1 - {output.shape}')
                # print(f'feature map1 - {feature_maps[j].shape}')
                output = torch.cat([output, feature_maps[j]],1)
                # output = asc(output, feature_maps[j])
        ...
        for i in range(16):
            ...
            for j in range(len(sec_feature_maps) - 1):
                # asc = ASC(output.shape[1]+sec_feature_maps[j].shape[1])
                print(f'output2 - {output.shape}')
                print(f'feature map2 - {sec_feature_maps[j].shape}')
                output = torch.cat([output, sec_feature_maps[j]],1)         
                # output = asc(output, sec_feature_maps[j])
