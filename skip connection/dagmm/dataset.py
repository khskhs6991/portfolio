...
class DAGMM_dataset(Dataset):
   def __init__(self, data, label, mode):
      ...
      for i in range(self.data.shape[0]): # 400
        for j in range(self.data.shape[1]): # 512
            if self.label[i][j] == 0:
                continue
   ...
   def __getitem__(self, index):
      ...
      img_pixel = (img_pixel - np.min(self.data)) / (np.max(self.data) - np.min(self.data))
      img_pixel = torch.from_numpy(img_pixel)
      img_pixel = img_pixel.reshape(-1, 224)
      ...
      return img_pixel, label_pixel
   

class Indian_pines_dataset(Dataset):
    def __init__(self, img, gt, mode, patch_size, normal, ratio = 0):
        super().__init__()
        random.seed(42)
        self.img = img
        self.gt = gt
    ...
    def __getitem__(self, index):
        ...
        img_pixel = self.img[center_pixel[0], center_pixel[1]]
        img_pixel = (img_pixel - np.min(self.img)) / (np.max(self.img) - np.min(self.img))
        # gt_pixel = self.gt[center_pixel[0] - self.boundary:center_pixel[0] + self.boundary + 1, center_pixel[1] - self.boundary:center_pixel[1] + self.boundary + 1]
        gt_pixel = self.gt[center_pixel[0], center_pixel[1]]
        # for i in range(self.d):
        #    for j in range(self.d):
        #       if gt_pixel[i][j] == self.normal:
        #          gt_pixel[i][j] = 1
        #       else:
        #          gt_pixel[i][j] = 0
